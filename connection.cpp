...
void Connection::draw(QPainter& P, double f) const
{
    ...
    QPolygonF polyArrow;
    polyArrow << pt2;
    pt1 = QPointF(pt2.x() + m_cArrowSize * qCos(ang + M_PI/6), pt2.y() - m_cArrowSize * qSin(ang + M_PI/6));
    polyArrow << pt1;
    pt1 = QPointF(pt2.x() + m_cArrowSize * qCos(ang - M_PI/6), pt2.y() - m_cArrowSize * qSin(ang - M_PI/6));
    polyArrow << pt1;
...
void Connection::update(double f)
{
    ...
    for (int i = 1; i < poly.count(); ++i) {
        ...
        if (qFuzzyCompare(pt1.x(), pt2.x()) == true)
            m_vAreas.append(QRectF(pt1.x() - m_cInflateSize / 2, pt1.y(), m_cInflateSize, pt2.y() - pt1.y()));
        else
            m_vAreas.append(QRectF(pt1.x(), pt1.y() - m_cInflateSize / 2, pt2.x() - pt1.x(), m_cInflateSize));
    }
}
